...
public class MarcaServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Part marcaPart = request.getPart("marcadagua");

        Path marcaDir = Files.createTempDirectory("ocr-temp");
        File marcaFile = new File(marcaDir.toFile(), "marcaFile.png");

        try (InputStream inputStream = marcaPart.getInputStream()) {
            Files.copy(inputStream, marcaFile.toPath());
        } catch (IOException e) {
            response.getWriter().write("Erro ao copiar o arquivo: " + e.getMessage());
            return;
        }

        Edicao.marcaDagua(imageFile, marcaFile);
